...
int yes=1,n;
  ...
  char * my_required_value;
	...
	{	
		...
		strcpy(srvip,ipstr);
		printf("The Server 3 has the port number %d and ip address %s.\n",ntohs(sin.sin_port),ipstr);
	...
	{ 
		...
		if ((bytesRecvd = recv(new_fd, buf, BUFFSIZE-1, 0)) == -1)
   		...
   		printf("The Server 3 has received a request with key %s from the Server 2 with port number %s and IP address 127.0.0.1.\n",my_key,SRV2); 
		...
		// Concatenate POST with value
		 my_post_val_s2=concat_p(s2,my_required_value);
			 ...
			 printf("The Server 3,sent reply %s to the Server 2 with port number %s and IP address 127.0.0.1\n",my_post_val_s2,SRV2);
